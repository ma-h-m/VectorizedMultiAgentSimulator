"""
核心类，调用pytorch，将处理的内容向量化，从而达成SIMD的效果。
实现了各种物理量的计算
...
## A super class, turning the data into shape compatible with torch
class TorchVectorizedObject(object):
    ...
    ## get and set the batch dimension
    @property
    def batch_dim(self):
    ...
    def device(self, device: torch.device):
        ...
        self._device = device
    ## check the batch_index whether legal or not
...
## Shape: Sphere
## The definition is just like the previous one
class Sphere(Shape):
...
## One Subclass of TorchVectorizedObject
## Describing the state of entities
## By putting multiple entities together as a vector, we can accelerate the calculation of computing.
class EntityState(TorchVectorizedObject):
    ...
    # set and get angular velocity 角速度 
    @property
    def ang_vel(self):
...
## TorchVectorizedObject:EntityState:AgentState
## Agent, one kind of entities
class AgentState(EntityState):
    def __init__(
        self,
    ):
        super().__init__()
        # communication utterance ##是否可以发言（交流）
        self._c = None
...
class Action(TorchVectorizedObject):
    def __init__(
        ...
        super().__init__()
        # physical motor noise amount ## 物理引擎噪声量（就是选择action a后会有一定噪声？）
        self._u_noise = u_noise
        # control range               ## 控制范围
        self._u_range = u_range
        # agent action is a force multiplied by this amount
        self._u_multiplier = u_multiplier   ## 将force乘一常量，物理意义大致就是质量的倒数？

        # physical motor noise amount   ## 旋转的噪声量
        ...
        # physical action   ## 位移的action
        self._u = None
        # rotation action   ## 旋转的action
        self._u_rot = None
...
## Using class Entitystate to create an observable class
...
## The definition is just as entities
...
class World(TorchVectorizedObject):
    def __init__(
        ...
        # simulation timestep
        self._dt = dt
        self._substeps = substeps   ## 仿真的timestep被拆分成几个子步
