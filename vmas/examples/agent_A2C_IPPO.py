...
class PPOmemory:
    ...
    # 每一步都存储trace到memory
    def push(self, states,next_states, action, prob, val, next_vals, rewards, done):
        ...
        for sta,ne_sta,act,pro,va,nva,rew,don in zip(states,next_states, action, prob, val, next_vals, rewards, done):
...
class Agent:
    ...
    def choose_action(self, state):
        ...
        value = [self.critic(x).detach() for x in state]  # state value值
        action_rec = []
        prob_rec = []
...
def get_args():
    ...
    parser.add_argument('--n_epochs', default=40, type=int, help='update number')
...
def train(cfg, env, agent, max_steps):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        while (not done.all()) and steps < max_steps :
            ...
            next_vals = [agent.critic(x).detach() for x in state_]
            rewards = sum(reward)

            for act,pro,rew,sta,sta_,va,neva in zip(action,prob,reward,state,state_,val,next_vals):

                agent.memory.push(sta[mask],sta_[mask], act[mask], pro[mask], va[mask], neva[mask], rew[mask], done[mask])
            mask = done == 0
