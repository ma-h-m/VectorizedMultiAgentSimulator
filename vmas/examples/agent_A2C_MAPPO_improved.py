...
def train(cfg, env, agent, max_steps):
    ...
    for i_ep in range(cfg.train_eps):
        ...
        while (not done.all()) and steps < max_steps :
            ...
            if steps % cfg.batch_size == 0:
                ...
                # end.record()
                # torch.cuda.synchronize()
                # print('learning once has cost',  start.elapsed_time(end))
            state = state_
    ...
    print('完成训练！')
    with open('ouput.txt','a') as f:    #设置文件对象
        f.write('Algorithm:' + cfg.algo_name + 'rewards:\n'+ str(rewards_rec) + str('\n'))
        f.close()
